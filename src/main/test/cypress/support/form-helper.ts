...
export const testFormError = (error: string): void => {
  cy.getByTestId("spinner").should("not.exist");
  cy.getByTestId("form-error").should("contain.text", error);
};

export const testHttpCallsCount = (counts: number): void => {
  cy.get("@request.all").should("have.length", counts);
};
